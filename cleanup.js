...
function s3Client() {
  if (env.driver === 'S3') {
    return new S3Client({ region: env.s3.region, credentials: { accessKeyId: env.s3.accessKeyId, secretAccessKey: env.s3.secretAccessKey } });
  } else {
...
function bucket() { return env.driver==='S3' ? env.s3.bucket : env.r2.bucket; }
...
(async ()=>{
  for (const row of expired) {
    try {
      await client.send(new DeleteObjectCommand({ Bucket: bucket(), Key: row.ukey }));
    } catch(e) {
      // ignore
    }
    delStmt.run(row.id);
