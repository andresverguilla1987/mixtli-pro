(() => {
  ...
  const cfg = Object.assign({
    apiBase: '', usersApi: '', grafana: '', prometheus: '', status: './status/',
    downloadSuite: '', downloadAddons: '', otelEndpoint: 'http://localhost:4318', cacheBust: true, theme:'green'
  }, window.MIXTLI_CFG || {});
  for (const k of ['apiBase','usersApi','grafana','prometheus','status','downloadSuite','downloadAddons','theme']) {
  ...
  async function pingHealth() {
    ...
    const t0 = performance.now(); let ok = false, rid = '—', ms = 0;
    try {
      const res = await fetch(url, { headers: { 'X-Request-Id': 'preview-' + Date.now() }});
      ms = Math.round(performance.now() - t0); ok = res.ok; rid = res.headers.get('x-request-id') || rid;
    } catch (e) { ms = Math.round(performance.now() - t0); ok = false; }
    $('api-status').className = 'dot ' + (ok ? 'green' : 'red'); $('api-latency').textContent = ok ? `(${ms} ms)` : '(sin respuesta)'; $('reqid').textContent = rid;
  }
