...
router.post('/login', async (req, res) => {
  const user = req.user;
  await sendTemplate(user.email, 'login_new_device', 'Nuevo inicio de sesión', {
...
router.post('/twofa-enabled', async (req, res) => {
  const user = req.user;
  await sendTemplate(user.email, 'twofa_enabled', 'Autenticación en dos pasos activada', {
    name: user.name || user.email,
    app_name: process.env.APP_NAME || 'Mixtli',
...
router.post('/password-reset-completed', async (req, res) => {
  const user = req.user;
  await sendTemplate(user.email, 'password_reset_completed', 'Tu contraseña se restableció', {
    name: user.name || user.email,
    app_name: process.env.APP_NAME || 'Mixtli',
