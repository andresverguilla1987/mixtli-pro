// Mixtli Mini 1.15.3 — backend completo (mínimo viable, R2/S3)
...
const DEFAULT_READ_TTL = Math.min(Number(process.env.DEFAULT_READ_TTL || 300), 86400); // 5m..24h
...
const clampTTL = (ttl) => Math.max(60, Math.min(Number(ttl || DEFAULT_READ_TTL), 86400));
function makeKeyFromFilename(name) {
  const id = Math.random().toString(16).slice(2, 8) + Math.random().toString(16).slice(2, 8);
  return `${Date.now()}_${id}_${name}`;
}
...
// ====== Routes ======
app.get('/', (_, res) => res.type('text').send('Mixtli Mini'));
app.get('/version', (_, res) =>
  res.json({ ok: true, name: 'Mixtli Mini', version: '1.15.3',
    env: { s3Configured: !!(S3_ENDPOINT && S3_BUCKET), region: S3_REGION, pathStyle: S3_FORCE_PATH_STYLE } })
);
app.get('/api/health', (_, res) => res.json({ ok: true, auth: !!JWT_SECRET, s3: !!(S3_ENDPOINT && S3_BUCKET) }));
...
// 2) COMMIT: guarda metadata en SQLite
...
// 3) READLINK: GET firmado (privado) con TTL
app.get('/api/readlink', auth(false), async (req, res) => {
  const key = req.query.key;
  const ttl = clampTTL(req.query.ttl);
  if (!key) return res.status(400).json({ ok: false, error: 'key requerido' });
...
// 4) THUMBNAIL: lee con GET firmado, genera y sube a thumbs/<key>.jpg
app.post('/api/thumbnail', auth(false), async (req, res) => {
  const { key, width = 480, watermark = {} } = req.body || {};
  if (!key) return res.status(400).json({ ok: false, error: 'key requerido' });

  // Descarga con presign GET (privado)
  const getUrl = await getSignedUrl(s3, new GetObjectCommand({ Bucket: S3_BUCKET, Key: key }), { expiresIn: 300 });
  const r = await fetch(getUrl);
  if (!r.ok) return res.status(404).json({ ok: false, error: 'no se pudo leer origen' });
...
// ---- Compartir (7 días por defecto) ----
app.post('/api/share', auth(false), async (req, res) => {
  const { key, ttl = 604800, password = '' } = req.body || {};
  if (!key) return res.status(400).json({ ok: false, error: 'key requerido' });
  const token = crypto.randomBytes(16).toString('hex');
  const expires_at = nowSec() + Math.min(Number(ttl) || 604800, 604800); // máx 7d
...
// Página de share: redirige a GET firmado corto
app.get('/s/:token', async (req, res) => {
  const row = await db.get(`SELECT * FROM shares WHERE token=?`, [req.params.token]);
  ...
  const url = await getSignedUrl(s3, new GetObjectCommand({ Bucket: S3_BUCKET, Key: row.key }), { expiresIn: 300 });
  res.redirect(url); // link temporal (5 min) protegido; el token dura hasta 7d
});
