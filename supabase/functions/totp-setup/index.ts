...
serve(async (req) => {
  try{
    const { email, issuer } = await req.json();
    if (!email) return new Response(JSON.stringify({ ok:false, error:"email required" }), { status:200 });
    const secret = randBase32(32);
    await db.from("admin_totp").upsert({ actor_email: email, secret_base32: secret, enabled: true });
    const uri = `otpauth://totp/${encodeURIComponent(issuer||'Mixtli')}:%20${encodeURIComponent(email)}?secret=${secret}&issuer=${encodeURIComponent(issuer||'Mixtli')}`;
    return new Response(JSON.stringify({ ok:true, secret, uri }), { headers: { "content-type":"application/json" } });
  }catch(e){
