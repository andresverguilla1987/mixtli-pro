// share.routes.js
// Rutas /api/share: crear, resolver, listar, revocar (estado en memoria).
// Compatible con ESM. Coloca este archivo junto a server.js.
...
export default function createShareRouter({ s3, bucket, secret }) {
  ...
  router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      const rec = store.get(id) || payload; // si reinició, usa contenido del token
      ...
      if (store.has(id)) {
        store.get(id).downloads = (store.get(id).downloads || 0) + 1;
      }
