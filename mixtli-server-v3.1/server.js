...
// Purge (hard delete). If trashKey provided, delete that key. If prefix provided, delete all under prefix.
app.post('/api/purge', async (req,res)=>{
  try{
    const { trashKey, prefix } = req.body || {};
    if(!trashKey && !prefix) return res.status(400).json({ error:'trashKey or prefix required' });
    const delKey = async (k)=>{
      await s3.send(new DeleteObjectCommand({ Bucket: BUCKET, Key: k }));
    };
    if(trashKey){
      await delKey(trashKey);
    } else {
      const pref = norm(prefix);
      let token;
      do{
        const out = await s3.send(new ListObjectsV2Command({ Bucket: BUCKET, Prefix: pref, ContinuationToken: token, MaxKeys: 1000 }));
        for(const o of (out.Contents||[])){
          if(o.Key && !o.Key.endsWith('/')) await delKey(o.Key);
        }
        token = out.IsTruncated ? out.NextContinuationToken : undefined;
      } while(token);
    }
    res.json({ ok:true });
  }catch(e){ console.error('purge',e); res.status(500).json({error:String(e)}) }
});
...
// Albums listing under a base prefix (e.g., public/ or _private/)
app.get('/api/albums', async (req,res)=>{
  try{
    const base = norm(req.query.base || '');
    if(!base) return res.status(400).json({ error:'base required (e.g., public/ or _private/)' });
    let token;
    const albums = new Set();
    // First pass: discover immediate subprefixes (albums)
    do{
      const out = await s3.send(new ListObjectsV2Command({ Bucket: BUCKET, Prefix: base, Delimiter: '/', ContinuationToken: token, MaxKeys: 1000 }));
      for(const cp of (out.CommonPrefixes || [])){
        if(cp.Prefix && cp.Prefix !== base) albums.add(cp.Prefix);
      }
      token = out.IsTruncated ? out.NextContinuationToken : undefined;
    } while(token);

    const result = [];
    for(const pref of albums){
      // cover: read .mixtli-cover.json if exists, else pick first image
      let coverKey = null;
      try{
        const coverConfKey = `${pref}.mixtli-cover.json`;
        const obj = await s3.send(new GetObjectCommand({ Bucket: BUCKET, Key: coverConfKey }));
        const buf = await obj.Body.transformToByteArray();
        const txt = new TextDecoder().decode(buf);
        const conf = JSON.parse(txt);
        coverKey = (conf && conf.coverKey) ? conf.coverKey : null;
      }catch{}
      // count & size & fallback cover
      let token2, count=0, total=0, firstImg=null;
      do{
        const out2 = await s3.send(new ListObjectsV2Command({ Bucket: BUCKET, Prefix: pref, ContinuationToken: token2, MaxKeys: 1000 }));
        for(const o of (out2.Contents||[])){
          if(o.Key && !o.Key.endsWith('/')){
            count++; total += (o.Size||0);
            const ext = (o.Key.split('.').pop()||'').toLowerCase();
            if(!firstImg && ['jpg','jpeg','png','webp','gif','avif'].includes(ext)) firstImg = o.Key;
          }
        }
        token2 = out2.IsTruncated ? out2.NextContinuationToken : undefined;
      } while(token2);
      if(!coverKey) coverKey = firstImg;
      result.push({
        prefix: pref,
        name: pref.slice(base.length).replace(/\/$/,''),
        count, totalSize: total,
        coverKey,
        coverPublicUrl: coverKey ? publicUrlFor(coverKey) : null
      });
    }
    res.json({ base, albums: result });
  }catch(e){ console.error('albums',e); res.status(500).json({error:String(e)}) }
});

// Set album cover
app.post('/api/album-cover', async (req,res)=>{
  try{
    const { prefix, coverKey } = req.body || {};
    if(!prefix || !coverKey) return res.status(400).json({ error:'prefix & coverKey required' });
    const key = `${norm(prefix)}.mixtli-cover.json`;
    const body = Buffer.from(JSON.stringify({ coverKey }, null, 2));
    await s3.send(new PutObjectCommand({ Bucket: BUCKET, Key: key, ContentType:'application/json', Body: body }));
    res.json({ ok:true });
  }catch(e){ console.error('album-cover',e); res.status(500).json({error:String(e)}) }
});
