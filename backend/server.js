// Mixtli Mini Backend (CORS-enabled)
...
const createUser = db.prepare('INSERT INTO users (id,email,pass,plan) VALUES (?,?,?,?)');
const createUpload = db.prepare('INSERT INTO uploads (id,userId,bucket,ukey,size,mime,expiresAt) VALUES (?,?,?,?,?,?,?)');
const getUpload = db.prepare('SELECT * FROM uploads WHERE id=? AND userId=?');
const markUploaded = db.prepare("UPDATE uploads SET status='READY', etag=? WHERE id=?");

function s3Client() {
  if (env.driver === 'S3') return new S3Client({ region: env.s3.region, credentials: { accessKeyId: env.s3.accessKeyId, secretAccessKey: env.s3.secretAccessKey } });
...
const planMax = (plan) => plan==='PRO'?env.maxPro: (plan==='ENTERPRISE'?env.maxEnt:env.maxFree);
const badExt = (name) => ['.exe','.bat','.cmd','.scr','.ps1','.js','.vbs','.jar'].some(ext=> name.toLowerCase().endsWith(ext));

const app = express();
const origins = env.corsOrigins === '*' ? true : env.corsOrigins.split(',').map(s=>s.trim());
app.use(cors({ origin: origins, methods: ['GET','POST','OPTIONS'], allowedHeaders: ['Content-Type','Authorization'], maxAge: 600 }));
app.options('*', cors());
app.use(express.json({ limit: '5mb' }));

app.use((req,res,next)=>{ const rid = req.headers['x-request-id'] || cryptoRandom(); req.id = rid; res.setHeader('x-request-id', rid); next(); });
...
app.post('/auth/login', (req,res)=>{
  ...
  if (!u || !bcrypt.compareSync(password, u.pass)) return res.status(401).json({ error:'invalid credentials' });
...
function auth(req,res,next){
  ...
  try { req.user = jwt.verify(token, env.jwt); next(); } catch { return res.status(401).json({ error:'invalid token' }); }
}
...
app.post('/upload/presign', auth, async (req,res)=>{
  ...
  if (badExt(filename)) return res.status(400).json({ error:'forbidden file extension' });
  const id = nanoid();
  ...
  createUpload.run(id, req.user.sub, bucket(), key, size, (mime||'application/octet-stream'), expiresAt.toISOString());
...
app.post('/upload/complete', auth, (req,res)=>{
  const { uploadId, etag } = req.body || {};
  const u = getUpload.get(uploadId, req.user.sub);
  if (!u) return res.status(404).json({ error:'upload not found' });
  markUploaded.run(etag || null, uploadId);
  res.json({ ok:true, uploadId });
});

app.get('/upload/:id/link', auth, async (req,res)=>{
  const u = getUpload.get(req.params.id, req.user.sub);
  ...
  const url = await getSignedUrl(s3Client(), new GetObjectCommand({ Bucket: u.bucket, Key: u.ukey }), { expiresIn: 60*60 });
  res.json({ url, expiresIn: 3600 });
});

app.listen(env.port, ()=> logger.info({ port: env.port, driver: env.driver }, 'up'));

function cryptoRandom(){ const arr = new Uint8Array(16); crypto.getRandomValues(arr); return [...arr].map(b=>b.toString(16).padStart(2,'0')).join(''); }
