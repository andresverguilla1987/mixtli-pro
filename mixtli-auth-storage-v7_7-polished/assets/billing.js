(() => {
  ...
  plansEl.addEventListener('click',(e)=>{
    ...
    const price=p.price[countrySel.value]; const out=document.getElementById('out-'+id);
    if(btn.dataset.act==='link'){ const url=`https://pay.mixtli.com/checkout?pid=${id}&cur=${countrySel.value}&amt=${price}`; out.textContent='Link configurado: '+url; cryptoEl.textContent=`≈ ${(price/cfg.cryptoRates.BTC).toFixed(6)} BTC • ${(price/cfg.cryptoRates.ETH).toFixed(5)} ETH • ${(price/cfg.cryptoRates.USDC).toFixed(2)} USDC`; }
    else if(btn.dataset.act==='sim'){ out.textContent='Compra simulada ✔ Recibiste '+p.gb+' GB.'; }
    else if(btn.dataset.act==='wallet'){ out.textContent='Saldo insuficiente (demo). Usa Simular compra.'; }
  });
  countrySel.onchange=render; render();
  document.getElementById('modeLabel').textContent = (cfg.mode||'demo').toUpperCase();
})();